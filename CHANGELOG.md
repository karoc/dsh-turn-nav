...
- **Aggressive stall protection for very long conversations** (hundreds of turns): initial auto-load pulls only a few pages (`MAX_AUTO_PAGES = 5`), and the rail-top scroll loads at most 3 pages per continuous pass (scroll away and back to load more). Every page prepend re-renders the whole conversation flow, so capping total prepends keeps the UI — including the rail itself — responsive even on ~150-turn sessions.
