...
- **Turn extraction**: the rail reads turns from the new `useChat` hook (ChatSnapshot) — `navigation.items()` for the loaded turns (each with turn number, prompt, anchor key), plus `timeline` for timestamps/status. On pre-0.1.2 dsh it also reads the full persisted history via the `sessions.history` RPC. Turns without a user message fall back to their first node's kind.
- **Jump-to-turn**: locate the turn's first chat-node key, find the DOM row via `data-chat-anchor-key="<key>"`, compute its position in the `[data-conversation-scroll]` scrollport, and set `scrollTop` precisely (more predictable than `scrollIntoView`). If the target row is not yet rendered (older page not loaded), it extends the window one page at a time through the official session store (`loadOlder`, waiting on its authoritative `hasMore`) and retries until the row appears — no "scroll once first" friction.
- **History-as-data (no flow prepends on open)**: the conversation window only materializes a page of events as DOM, and extending it (`loadOlder`) re-renders the whole flow — expensive on long sessions. The rail instead reads the full persisted history as plain data and derives every turn from it, so opening a session never touches the flow DOM. On dsh 0.1.2+ this uses the Typert Remote `session/page` channel (the same persisted log the official window reads, paged through `ctx.remote.session` — the namespace is mounted by the base web assembly, so **no host changes, no new dependencies**); on older dsh it uses the `sessions.history` RPC. **On demand**: clicking a capsule for a turn already in the window scrolls to it directly; for a turn outside the window, the rail extends the window page by page (via the official session store's `loadOlder`, or the "Load earlier" button on older hosts) until that turn is in the window, then scrolls and highlights it — the only path that prepends into the flow, and it runs only when the user clicks.
