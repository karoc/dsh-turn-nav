...
async function main() {
  ...
  await page.evaluate(() => {
    ...
    // Prefer the 5-turn dev session (this script hovers the 3rd capsule); fall
    // back to any non-empty session.
    let target = rows.find((r) => (r.textContent || '').includes('dsh 系统服务化'))
    target ??= rows.find((r) => {
